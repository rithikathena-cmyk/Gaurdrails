...
`EntityRail`/`PIIRail` do the classifying (Presidio, the judge, the built-in
regex recognizers); this file is about what happens once a kind is known —
whether PERSON gets redacted while GOVERNMENT and ORGANISATION are left
alone on the same text, and whether the deterministic, checksum-backed
structured-PII path (email, phone, Aadhaar, PAN) still runs and is still
protected regardless of what the NER kinds are configured to do. The agent
that classifies never decides an action itself — every verdict below comes
from `action_verdict()` + `kind_actions.resolve()`, the same two functions
the engine already used for a single global action per surface.
"""
...
# ── Test 2: structured PII stays detected and protected ────────────────
def _pii_rail(allowlist=(), **kind_actions: str) -> PIIRail:
    return PIIRail(
        entities=["EMAIL_ADDRESS", "PHONE_NUMBER", "AADHAAR", "PAN"],
        confidence_threshold=0.5, mask_strategy="vault-token",
        partial_reveal=0, custom_regex=[], vault=Vault(),
...
def test_structured_pii_kinds_are_still_found_and_masked_by_default():
    ...
    text = ("Email meera@example.com, call 415-555-0143, Aadhaar 234123412346, "
           "PAN AAAPL1234C.")
    result = rail.evaluate(text, "mask", blank("pii.detect"))
    assert result.verdict is Verdict.MASK
...
def test_one_structured_kind_can_be_relaxed_without_affecting_the_others():
    ...
    per-kind override reaches the deterministic recognizers too, not just
    the NER kinds."""
    rail = _pii_rail(PHONE_NUMBER="flag")
...
def test_an_allowlisted_phone_number_stays_visible_even_if_its_kind_is_masked():
    ...
    rail = _pii_rail(allowlist=[r"1800[\s-]?425[\s-]?1969"], PHONE_NUMBER="mask")
    result = rail.evaluate(text, "mask", blank("pii.detect"))
...
def test_two_structured_kinds_render_with_different_strategies_in_one_call():
    rail = PIIRail(
        entities=["EMAIL_ADDRESS", "PHONE_NUMBER"], confidence_threshold=0.5,
        mask_strategy="vault-token",   # the global default, overridden for one kind
        partial_reveal=0, custom_regex=[], vault=Vault(),
        kind_mask_strategy={"PHONE_NUMBER": "redact"},
    )
    text = "Email meera@example.com, call 415-555-0143."
    result = rail.evaluate(text, "mask", blank("pii.detect"))
...
def test_a_kind_mask_strategy_override_does_not_change_whether_it_is_masked():
    ...
    *action*, which stays whatever pii.kind_actions (or the surface
    ...
    rail = PIIRail(
        entities=["PHONE_NUMBER"], confidence_threshold=0.5,
        mask_strategy="vault-token", partial_reveal=0, custom_regex=[], vault=Vault(),
