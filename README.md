...
```
guardrails/            core library — no HTTP, no UI
  registry.py          every parameter declared once; the source of truth
  config.py            YAML loading, validation, overrides layer
  types.py             Verdict, RailResult, Trace, verdict precedence
  tracing.py           Tracer + hash-chained AuditLog
  engine.py            the orchestrator — evaluate(), converse(), ingest()
  agent.py             tool loop, agent.tool / agent.data rails, approval gate
  ingest.py            extraction, chunking, the corpus and its BM25 index
  scenarios.py         five end-to-end scenarios that assert on real runs
  llm.py               Claude client: judges, generation, agent turns
  knowledge.py         seed corpus, and the retrieval entry point
  rails/
    normalize.py       NFKC + homoglyph fold (locked on)
    words.py           Aho–Corasick automaton, pure Python
    pii.py             recognizers, checksums, AES-256-GCM vault
    content.py         content safety + prompt injection
...
server/                HTTP only — holds no guardrail logic
  ...
  routes/
    ...
    params.py          registry read + validated edits

web/                   no build step, ES modules
  home.html            the landing page and the pipeline diagram        (/)
  login.html           split-screen sign-in, with quick role tiles (/login)
  index.html           the console itself                        (/console)
  styles/
    tokens.css         palette and type, both themes
    app.css            layout and components
  scripts/
    api.js             the only module that talks to the server
    dom.js             shared helpers
    chat.js  docs.js  trace.js  params.js  markdown.js  main.js

demo/                  the pipeline, drawn
  index.html           swimlane diagram, chat ↔ agent toggle, scenario runner  (/demo)
  stages.html          the same request stage by stage                  (/demo/stages)
  flow.py              the chat pipeline in the terminal, with a live trace
  home_diagram.py      computes the geometry of the home page flowchart
  README.md            the written account, with a mermaid chart

config/
  policy.yaml          the baseline you edit by hand
  overrides.yaml       generated by the console (commit it)
  lexicons/

data/
  corpus.json          ingested documents (generated, gitignored)

eval/
  suite.yaml           labelled cases: retrieval, rails, answers
  report.json          generated by --eval (gitignored)

tests/                 309 tests, no API key needed
  test_checksums.py  test_words.py  test_pii.py     test_ingest.py
  test_config.py     test_engine.py  test_registry.py  test_agent.py
  test_explain.py    test_llm.py     test_parameters.py  test_api.py
  test_eval.py       test_agent.py
```

The frontend hardcodes nothing about the control surface. Families, surfaces, severity
levels, lock categories and their theme colours, which control each parameter type needs,
and the sample prompts all come from `/api/parameters` and `/api/samples` — which are
generated from the Python registry. Add a parameter to `registry.py` and it appears in the
UI with the right control, no frontend edit.
